...
class BezierRenderer:
    ...
    def render_save_bezier_single_grid(self, points, output_path, rows=4, cols=4, scale=224):
        """
        渲染render_beziers单个贝塞尔曲线并将结果保存为网格形式的图像文件。

        参数:
        - points: tensor[batch_size, num_points, 2],在01之间
        ...
        """
        # 使用贝塞尔渲染器生成图像
        img = self.render_beziers(points * scale)
        print(img.shape)
        # 重新排列张量，将它转换为 [16, 224, 224]，去掉最后的单通道维度
        tensor = img.clone().cpu().detach().numpy().squeeze(-1)  # 去掉最后的单通道维度
        ...
        for i in range(rows):
            for j in range(cols):
                idx = i * cols + j
                ax = axes[i, j]
                ax.imshow(tensor[idx], cmap='gray')  # 使用灰度色图显示单通道图像
                ax.axis('off')  # 关闭坐标轴
