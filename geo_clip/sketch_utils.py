...
# 绘制整张图片
def render_img_rgb_from_renderer(points, renderer,sampled_points_list,epoch,save_path):
    """
    使用renderer渲染完整的草图
    ...
    if epoch % 10 == 0:
        save_dir = os.path.join(save_path, f"{epoch}flamingo.png")
        # print(img2.shape)
        img2 = img.clone().detach().cpu().numpy()
        # 使用 matplotlib 绘制图像
        plt.imshow(img2)

        # 生成一组视觉区分度高的颜色（使用matplotlib的tab10色环）
        colors = plt.cm.get_cmap('tab20', 16)
        sampled_points_list = sampled_points_list.clone().detach().cpu().numpy()
        # print(sampled_points_list.shape)
        for i in range(sampled_points_list.shape[0]):
            points = sampled_points_list[i]  # shape: [n, 2]
            color = colors(i)

            # 画曲线并加 label
            plt.scatter(points[:, 0] * 224, points[:, 1] * 224,
                        color=color,
                        linewidth=0.5,
                        label=f'Curve {i}')  # 图例中标注编号

        # 显示图例
        plt.legend(loc='upper right', fontsize=8, title='Curve Index')
...
def render_img_rgb_from_infer(points,canvas_height,canvas_width ,epoch,save_path):
    """
    使用renderer渲染完整的草图
    ...
    scene_args = pydiffvg.RenderFunction.serialize_scene( \
        canvas_width, canvas_height, paths, shape_groups)
