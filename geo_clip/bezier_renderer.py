...
class BezierRenderer:
    ...
    def render_beziers_and_similarity(self,epoch ,shape_context_matrix,sampled_points_list=None):
        sampled_points_clone=sampled_points_list.clone().detach().cpu().numpy()
        # 颜色列表
        colors = plt.cm.tab20.colors  # 使用 Matplotlib 的颜色映射
        fig, ax = plt.subplots(figsize=(10, 8), dpi=100)  # **保证 224x224 分辨率**
        for num, sampled_points in enumerate(sampled_points_clone):
            # sampled_points 是形状为 (10, 2) 的数组，其中包含 10 个 (x, y) 坐标
            sampled_x, sampled_y = (sampled_points[:, 0] * 224), (
                        sampled_points[:, 1] * 224) # 提取 x 和 y 坐标
            # 在曲线的中间点旁边添加标签
            mid_x, mid_y = sampled_x[len(sampled_x) // 2], sampled_y[len(sampled_y) // 2]
            ax.text(mid_x, mid_y, f'Curve {num + 1}', color='black', fontsize=10, verticalalignment='bottom',
                     horizontalalignment='right')

            # 绘制每条曲线的采样点
            ax.plot(sampled_x, sampled_y, 'o-', color=colors[num % len(colors)], label=f'Curve {num + 1}')  # 使用圆点和线连接

        # 设置图形标题和标签
        ax.set_title(f'Sampled Points for Bezier Curves {epoch}')
        ax.set_xlabel('X')
        ax.set_xlabel('Y')
        ax.legend(loc='best')
        # **将 Figure 转换为 NumPy 数组**
        fig.canvas.draw()
        img_np = np.array(fig.canvas.renderer.buffer_rgba())  # (H, W, 4) 形状 (RGBA)
        plt.close(fig)  # 关闭 Figure，释放资源

        # **转换为 RGB (去掉 Alpha 通道)**
        img_np = img_np[:, :, :3]  # (224, 224, 3)

        # **转换为 PyTorch Tensor**
        img_tensor = torch.from_numpy(img_np).permute(2, 0, 1).float() / 255.0  # (3, 224, 224)



        plt.figure(figsize=(10, 8), dpi=100)
        # 转换为numpy数组
        shape_context_matrix_np = shape_context_matrix.clone().detach().cpu().numpy()
        # 绘制热力图
        im = plt.imshow(shape_context_matrix_np, cmap='YlOrRd', interpolation='nearest')

        # 添加数值标注,只标注非零值
        for i in range(shape_context_matrix_np.shape[0]):
            for j in range(shape_context_matrix_np.shape[1]):
                plt.text(j, i, f'{shape_context_matrix_np[i, j]:.2f}',
                         ha='center', va='center',
                         color='black' if shape_context_matrix_np[i, j] < 0.7 else 'white',
                         fontsize=8)

        # 添加颜色条和标题
        plt.colorbar(im)
        plt.title(f'shape_context_matrix - Epoch {epoch}')
        plt.savefig('text.png')
        # **转换为 NumPy 图像并移除 Alpha 通道**
        plt.tight_layout()
        sc_sim_img = np.array(plt.gcf().canvas.renderer.buffer_rgba())  # (H, W, 4)
        sc_sim_img = sc_sim_img[:, :, :3]  # 保留 RGB 通道，去掉 Alpha 通道

        # **转换为 PyTorch Tensor (3, H, W) 格式**
        sc_sim_img = torch.from_numpy(sc_sim_img).permute(2, 0, 1).float() / 255.0  # (3, H, W)

        # +++ 关键清理步骤 +++
        plt.close(fig)  # 关闭并释放 figure 内存
        plt.cla()  # 清除当前 axes
        plt.clf()  # 清除当前 figure
        plt.close('all')  # 关闭所有 matplotlib 窗口
        return img_tensor,sc_sim_img
    ...
    def mask_img(self, control_points):
        ...
        for img_mask in img_beziers:
            ...
            if mask_indices is not None:
                ...
                cropped = img_mask * rect_mask
                cropped = cropped[y_min:y_max + 1, x_min:x_max + 1, :]
            else:
                cropped = img_mask * torch.zeros_like(reverse_mask)
            ...
            cropped = F.resize(cropped, (224, 224))
