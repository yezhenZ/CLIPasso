...
class PainterOptimizer:
    def __init__(self, args,gcn_parameters, renderer):
    ...
    def init_optimizers(self):
        self.points_optim = torch.optim.Adam([
            ...
            # {'params': self.gcn_parameters, 'lr': 0.0001}
        ])
        if self.gcn_parameters is not None:
            # 创建一个独立的优化器，仅包含 gcn_parameter
            self.gcn_optim = torch.optim.Adam(self.gcn_parameters, lr=0.001)
            # 为 gcn_parameter 创建独立的学习率调度器
            self.gcn_lr_scheduler = torch.optim.lr_scheduler.StepLR(self.gcn_optim, step_size=500, gamma=0.1)
    ...
    def update_lr(self, counter):
        ...
        # 调用gcn_optim的调度器更新gcn_parameter的学习率
        self.gcn_lr_scheduler.step(counter)  # 每个epoch调用一次
