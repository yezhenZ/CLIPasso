...
def main(args):
    ...
    for epoch in epoch_range:
        ...
        points_rare=points.clone().detach()
        # 生成掩码图像
        ...
        # 计算形状上下文
        sampled_points_list = bezier.batch_sample(control_points=points)
        #  #【curves,samples,直方图】
        shape_context_histograms = shape_context_2.compute(sampled_points_list)
        shape_context_matrix = shape_context_2.compute_similarity(shape_context_histograms)

        SP_adjmatrix = shape_context_2.find_top_k_similar_curves(shape_context_matrix, shape_context_histograms, k=3)

        #gcn
        new_points = GCNmodel(feature, SP_adjmatrix).view(-1, 4, 2)
        ...
        loss.backward()
        # if epoch >= epoch_gcn_start:
        #     # print(SP_adjmatrix.requires_grad)
        #     #获取控制点
        #     for i, path in enumerate(renderer.shapes):
        #         print(path.points)
        #         print("Gradient values:\n", path.points.grad)
        #     for name, param in GCNmodel.named_parameters():
        ...
        #     # writer.add_image(f'{epoch}new_points_grid', new_points_grid)
        #
        #     # 保存为 .pt 文件（PyTorch 张量格式）
        #     # torch.save(feature, f"{args.output_dir}/feature.pt")
        #     # 假设 model 是你的模型
        #     # torch.save(GCNmodel.state_dict(), 'gcn_parameters.pth')
